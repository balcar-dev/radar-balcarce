...
| 7 | **Cada nota dice quién la escribió** (IA o fuente, automática o revisada), y el sitio no promete una revisión que no hubo. | Prueba (desde el 25/09): `pruebas/seo-paginas.test.mjs` ("el autor de los datos estructurados dice lo mismo que la firma de la nota", "no volvemos a prometer una revisión humana que no hay"). La firma visible es el componente `<Firma>` (`CRITERIO-EDITORIAL.md`, sección 10). |
