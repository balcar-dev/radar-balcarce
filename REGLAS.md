...
| 6 | **Lo que escribe la IA se verifica contra la fuente.** Si inventó un número, un nombre, un día o una cita, no se usa. | Prueba: `pruebas/verificar.test.mjs` (número, nombre, día, cita, delito como hecho, negaciones) y `pruebas/reescritura.test.mjs`. |
| 7 | **Cada nota dice quién la escribió** (IA o fuente, automática o revisada). | Nada automático: es un componente de la web (`<Firma>`). Está en `EDITORIAL.md`. |
...
| 9 | **Nunca identificar a un menor ni a una víctima.** El semáforo rojo lo frena. La lista no se toca sin preguntar. | Prueba: `pruebas/notas.test.mjs` (casos que dan rojo) y `pruebas/fuentes.test.mjs` (el semáforo tiene su lista de rojo). |
...
| 18 | **`ingesta/`, `panel/` y `redes/` no importan nada de afuera de Node.** | Prueba: `pruebas/fuentes.test.mjs` ("el motor no necesita nada instalado"). |
...
1. Escribirla acá con su número.
